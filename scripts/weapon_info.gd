...
const WEAPONS := {
	...
	"frost_lance": {"built": true, "script": "res://scripts/weapons/frost_lance.gd",
		"usage": ["HOLD LMB  charge (1.4s), RELEASE  fire the beam (350m)", "Every hit CHILLS: 30% slower for 1.8s", "Full charge also FREEZES for 0.5s", "1.5s cooldown", "Small lock circle"],
		...
		"summary": "Charged beam that CHILLS (-30% speed for 1.8s); a full charge freezes for 0.5s.",
		"combo": "Chill them, then land a RAILGUN or ARBALEST shot."},
	"gravity_well": {"built": true, "script": "res://scripts/weapons/gravity_well.gd",
		"usage": ["LMB  throw a singularity (lands, or pops open after 1s)", "Drags everyone within 34m hard into it for 3s", "No dashing out while it pulls", "Then it bursts", "6s cooldown"],
		...
		"summary": "Throw a singularity that drags players hard into it for 3s (no dashing out), then pops.",
	...
	"stasis_mine": {"built": true, "script": "res://scripts/weapons/stasis_mine.gd",
		"usage": ["LMB  lob a mine; it sticks where it lands", "Arms after a moment; anyone within 12m sets it off", "The 16m blast STAGGERS everyone in it", "Two out at once; a third replaces the oldest"],
	...
	"time_dilator": {"built": true, "script": "res://scripts/weapons/time_dilator.gd",
		"usage": ["LMB  spread a 25m bubble round you for 5s", "Enemies inside move AND fire at 40% speed", "Their shots crawl through it", "Turrets and targets inside stop dead", "10s cooldown"],
		...
		"summary": "A bubble round you where enemies move, fire and shoot at a crawl.",
	...
	"hunters_sigil": {"built": true, "script": "res://scripts/weapons/hunters_sigil.gd",
		"usage": ["LMB  tag the target nearest the crosshair: any range, through walls", "You see them through walls and on the minimap for 6s", "Tagged targets are MARKED for 3s (1.5x damage)", "4s cooldown"],
		...
		"summary": "Tag a target at any range: you see them through walls and on the minimap for 6s.",
		"combo": "Tag, then chase them down with TETHER or BLINK EDGE."},
	"executioner": {"built": true, "script": "res://scripts/weapons/executioner.gd",
		"usage": ["LMB  lunge and chop everything in front (9m)", "69 damage a chop, TRIPLE below 30% health", "1.2s cooldown"],
	...
	"echo_rifle": {"built": true, "script": "res://scripts/weapons/echo_rifle.gd",
		"usage": ["HOLD LMB  semi-auto hitscan (0.6s between shots)", "Every hit echoes 1s later for half damage", "Echoes follow the target wherever it went", "5-round magazine, 2.4s reload (T reloads early)", "Small lock circle"],
		...
		"summary": "Every hit echoes one second later for half damage.",
	...
	"splinter_bomb": {"built": true, "script": "res://scripts/weapons/splinter_bomb.gd",
		"usage": ["LMB  throw a shard that sticks to what it hits", "Homes hard on the lock", "Bursts 2s later", "Mark the target first for 1.5x", "2.5s cooldown"],
	...
	"winglets": {"built": true, "script": "res://scripts/weapons/winglets.gd",
		"usage": ["LMB in the air  flap: an extra jump (3 per trip)", "HOLD in the air  glide: a slow drift that speeds you up the way you look", "Flaps come back when you land"],
		"name": "WINGLETS", "tag": "GLIDER", "group": "skyborne",
		"color": Color(0.8, 1.0, 0.6), "layout": "Two long flat wings straight out to the sides.",
		"summary": "Flap for extra jumps in the air, and hold to glide where you look.",
		"combo": "Glide over them with RAILGUN or ARBALEST."},
	"meteor_drop": {"built": true, "script": "res://scripts/weapons/meteor_drop.gd",
		"usage": ["LMB in the air  slam straight down", "Huge damage; the crater grows with how far you fell", "LMB on the ground  leap up first", "2.5s cooldown"],
	...
	"shard_mortar": {"built": true, "script": "res://scripts/weapons/shard_mortar.gd",
		"usage": ["LMB  lob a shell at the crosshair in a high arc", "Bursts, then eight bomblets go off around it", "1.1s cooldown"],
	...
	"crystal_wall": {"built": true, "script": "res://scripts/weapons/crystal_wall.gd",
		"usage": ["LMB  raise a 30m x 14m wall 12m ahead for 5s", "Blocks shots and players", "8s cooldown"],
	...
	"arc_pylon": {"built": true, "script": "res://scripts/weapons/arc_pylon.gd",
		"usage": ["LMB  plant a pylon at your feet for 8s", "Zaps whoever is nearest within 96m", "One at a time; 10s cooldown"],
		"name": "ARC PYLON", "tag": "TESLA TURRET", "group": "fortress",
		"color": Color(0.85, 1.0, 0.3), "layout": "Two tall forks, one each side, pointing up.",
		"summary": "Plant a pylon that zaps anyone within 96m for 8s.",
		"combo": "Hold a room with SPIKE CARPET."},
	"spike_carpet": {"built": true, "script": "res://scripts/weapons/spike_carpet.gd",
		"usage": ["LMB  lay a 240m line of spikes ahead for 6s", "Anyone crossing it is cut and slowed", "7s cooldown"],
	...
	"crystal_saber": {"built": true, "script": "res://scripts/weapons/crystal_saber.gd",
		"usage": ["LMB  swing: a wide arc (8m) that KILLS in one cut", "For 0.6s after a swing, shots that hit you are DEFLECTED back", "1s cooldown"],
		...
		"summary": "One-cut-kill melee arc; mid-swing it deflects shots back where they came from.",
		"combo": "BLINK EDGE in and cut."},
	"shock_knuckle": {"built": true, "script": "res://scripts/weapons/shock_knuckle.gd",
		"usage": ["LMB  charge your fists (3s)", "Your next DASH becomes a punch", "Anyone you hit is KILLED in one punch and flies"],
		...
		"summary": "Charges your next dash into a one-punch-kill knockback punch.",
		"combo": "Punch them off the map."},
	"buzzsaw": {"built": true, "script": "res://scripts/weapons/buzzsaw.gd",
		"usage": ["HOLD LMB  spin the blades up", "Anyone touching you while it's spun up is KILLED", "Ram people"],
		...
		"summary": "Blades spin round the ball; one touch kills.",
		"combo": "Build speed with MOMENTUM weapons and ram."},
	"thorn_burst": {"built": true, "script": "res://scripts/weapons/thorn_burst.gd",
		"usage": ["HOLD LMB  charge (0.8s), RELEASE  burst", "120 shards all round you (45m)", "Longer charge, harder shards"],
		...
		"summary": "Release a 360-degree storm of shards all round you.",
		"combo": "TETHER into a crowd and burst."},
	"mirage": {"built": true, "script": "res://scripts/weapons/mirage.gd",
		"usage": ["LMB  two decoys in your colour roll off for 7s", "Enemy locks on you jump to your decoys", "Any hit pops a decoy", "12s cooldown"],
		"name": "MIRAGE", "tag": "DECOYS", "group": "brawler",
		"color": Color(0.5, 0.35, 0.7), "layout": "Two ghostly blades trailing behind at angles.",
		"summary": "Two decoy balls peel off and run; enemy locks on you jump to them.",
		"combo": "Decoy, flank, SCATTER."},
	# MARKSMAN / LONG RANGE
	"beam_lance": {"built": true, "script": "res://scripts/weapons/beam_lance.gd",
		"usage": ["HOLD LMB  a steady laser (300m)", "Burns hotter the longer it stays on one target (up to 3x)", "Overheats after 2s of firing, 2.5s to cool", "Small lock circle"],
	...
	"ricochet_disc": {"built": true, "script": "res://scripts/weapons/ricochet_disc.gd",
		"usage": ["LMB  throw a disc that homes on the lock", "Bounces off walls up to 3 times", "+35% damage after each bounce"],
		...
		"summary": "A homing disc that bounces off walls up to 3 times, hitting harder each bounce.",
	...
	"tears_of_an_angel": {
		...
		"layout": "Swarm's wings, two a side, raised behind the ball.",
